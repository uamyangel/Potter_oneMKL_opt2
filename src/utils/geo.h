...
public:
    ...
    void Update(const IntervalT& rhs) {
    ...
    bool Contain(const IntervalT& rhs) const { return low <= rhs.low && high >= rhs.high; }
    bool StrictlyContain(int val) const { return val > low && val < high; }
    bool StrictlyContain(const IntervalT& rhs) const { return low < rhs.low && high > rhs.high; }
...
public:
    ...
    void Expand(T val) { Expand(val, val); }
    void Expand(T xVal, T yVal) { 
        x.Expand(xVal);
        y.Expand(yVal);
    }

    void ExpandUnsigned(T xVal, T yVal) {
        x.ExpandUnsigned(xVal);
        y.ExpandUnsigned(yVal);
    }
    ...
    bool Contain(const T ptx, const T pty) const { return x.Contain(ptx) && y.Contain(pty); }
    bool StrictlyContain(const PointT<T>& pt) const { return x.StrictlyContain(pt.x) && y.StrictlyContain(pt.y); }
    PointT<T> GetNearestPointTo(const PointT<T>& pt) const { return {x.GetNearestPointTo(pt.x), y.GetNearestPointTo(pt.y)}; }
