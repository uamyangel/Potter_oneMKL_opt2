...
public:
	aStarRoute(Database& database_, bool isRuntimeFirst_) : database(database_), isRuntimeFirst(isRuntimeFirst_) {
		...
		auto nodeInfos_resize = [&](int tid) {nodeInfosForThreads[tid].resize(database.numNodes);};
		auto occChange_resize = [&](int tid) {occChangeForThreads[tid].resize(database.numNodes);};
